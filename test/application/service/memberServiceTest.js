    ...
    describe('#auth(memberID, traceContext, callback)', () => {
        context('auth lessee member from smartgrid-lessee microservice)', () => {
            it('return null if auth fail', done => {
                let mockSmartgridLesseeServiceGateway = {};
                mockSmartgridLesseeServiceGateway.authMember = (memberID, traceContext, callback) => {
                    callback(null, null);
                };
                muk(service, "_smartgridLesseeServiceGateway", mockSmartgridLesseeServiceGateway);
